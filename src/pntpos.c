...
/* satの数字を衛星番号に変換する----------------------------------------------*/
char* satnum2prn(int satnum)
{
	static char satnum_char[3];
	static char prn[4];
	if (satnum < 33)
	{
		snprintf(satnum_char, 3, "%02d", satnum);
		prn[0] = 'G';
		prn[1] = satnum_char[0];
		prn[2] = satnum_char[1];
	}
	//GLONASS
	else if (32 < satnum && satnum < 57)
	{
		satnum = satnum - 32;
		snprintf(satnum_char, 3, "%02d", satnum);
		prn[0] = 'R';
...
/* 残差を出力 -----------------------------------------------------*/
static int rescode_sub(int iter, const obsd_t *obs, int n, const double *rs,
				   const double *dts, const double *vare, const int *svh,
				   const nav_t *nav, const double *x, const prcopt_t *opt,
				   double *v, double *H, double *var, double *azel, int *vsat,
				   double *resp, int *ns)
                  ...
                  double *resp, char *msg)
{
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (norm(dx,NX)<1E-4) {
			...
			/* 残差を出力 */
			_ = rescode_sub(i,obs,n,rs,dts,vare,svh,nav,x,opt,v,H,var,azel,vsat,resp,
				   &ns);
